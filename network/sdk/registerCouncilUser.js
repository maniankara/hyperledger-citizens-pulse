...
async function main() {
  try {
    ...
    const adminIdentity = await wallet.get("admin1");
    if (!adminIdentity) {
      console.log(
        'An identity for the admin user "admin1" does not exist in the wallet'
      );
      console.log("Run the enrollCouncilAdmin.js application before retrying");
      return;
    }
    ...
    console.log(
      'Successfully registered and enrolled admin user "org1User" and imported it into the wallet'
    );
  } catch (error) {
