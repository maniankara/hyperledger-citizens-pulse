...
export PRIVATE_DATA_CONFIG=${PWD}/chaincodes/fabcar/go/collections_config.json

if [ "$CC_SRC_LANGUAGE" = "go" -o "$CC_SRC_LANGUAGE" = "golang" ] ; then
	CC_RUNTIME_LANGUAGE=golang
	CC_SRC_PATH="./chaincodes/fabcar/go/"
...
approveForMyOrg() {
  ...
  setGlobals $ORG 0
  set -x
  peer lifecycle chaincode approveformyorg -o localhost:7050 --ordererTLSHostnameOverride orderer.example.com --tls --collections-config $PRIVATE_DATA_CONFIG --cafile $ORDERER_CA --channelID $CHANNEL_NAME --name fabcar --version ${VERSION}  --package-id ${PACKAGE_ID} --sequence ${VERSION} >&log.txt
  set +x
  cat log.txt
...
checkCommitReadiness() {
  ...
  for i in {0..1}; do 
    ...
    while [ $rc -ne 0 -a $COUNTER -lt $MAX_RETRY ] ; do
      ...
      set -x
      peer lifecycle chaincode checkcommitreadiness --collections-config $PRIVATE_DATA_CONFIG --channelID $CHANNEL_NAME --name fabcar --version ${VERSION} --sequence ${VERSION} --output json  >&log.txt
      res=$?
      set +x
      let rc=0
      for var in "$@"
...
commitChaincodeDefinition() {
  ...
  set -x
  peer lifecycle chaincode commit -o localhost:7050 --ordererTLSHostnameOverride orderer.example.com --tls --collections-config $PRIVATE_DATA_CONFIG --cafile $ORDERER_CA --channelID $CHANNEL_NAME --name fabcar $PEER_CONN_PARMS --version ${VERSION} --sequence ${VERSION}  >&log.txt
  res=$?
  set +x
  cat log.txt
...
chaincodeInvokeInit() {
  ...
  set -x
#   peer chaincode invoke -o localhost:7050 --ordererTLSHostnameOverride orderer.example.com --tls --cafile $ORDERER_CA -C $CHANNEL_NAME -n fabcar $PEER_CONN_PARMS --isInit -c '{"Args":[]}' >&log.txt
  peer chaincode invoke -o localhost:7050 --ordererTLSHostnameOverride orderer.example.com --tls --cafile $ORDERER_CA -C $CHANNEL_NAME -n fabcar $PEER_CONN_PARMS  -c '{"function":"initMarble","Args":[]}' --transient "{\"marble\":\"$MARBLE\"}" >&log.txt
  res=$?
  set +x
  cat log.txt
...
chaincodeQuery() {
  ...
  for i in {0..1}; do
    ...
    while [ $rc -ne 0 -a $COUNTER -lt $MAX_RETRY ] ; do
      ...
      set -x 
      peer chaincode query -C $CHANNEL_NAME -n fabcar -c '{"function": "ReadMarblePrivateDetails","Args":["marble1"]}' >&log.txt
      res=$?
      set +x
      let rc=$res
      COUNTER=$(expr $COUNTER + 1)
