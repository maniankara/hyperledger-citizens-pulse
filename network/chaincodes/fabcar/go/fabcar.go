...
func (s *SmartContract) ModifyPrice(ctx contractapi.TransactionContextInterface, marbleID string) (*MarblePrivateDetails, error) {

	marbleJSON, err := ctx.GetStub().GetPrivateData("collectionMarblePrivateDetails", marbleID) //get the marble from chaincode state
	if err != nil {
			return nil, fmt.Errorf("failed to read from marble %s", err.Error())
		}
		if marbleJSON == nil {
		...
		marble := new(MarblePrivateDetails)
	_ = json.Unmarshal(marbleJSON, marble)

	marble.Price = marble.Price + 1
	
	marblePrivateDetailsAsBytes, err := json.Marshal(marble)
	if err != nil {
		return marble, fmt.Errorf(err.Error())
	}
	err = ctx.GetStub().PutPrivateData("collectionMarblePrivateDetails", marbleID, marblePrivateDetailsAsBytes)
	if err != nil {
		return marble, fmt.Errorf("failed to put Marble private details: %s", err.Error())
	}

	return marble, nil
}

func (s *SmartContract) UpdatePrivateData(ctx contractapi.TransactionContextInterface) error {
